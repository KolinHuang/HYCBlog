...
$$
以上公式为“算数几何均值不等式”，等号当且仅当${n_{1}=n_{2}=...=n_{a}}$时成立。

所以得出推论：

> 将绳子以相等的长度等分为多段，得到的乘积最大

设将绳子按长度x等分为a段，即${n=ax}$，则乘积为${x^a}$。所以有：
$$
x^a = x ^ \frac{n}{x} = (x^\frac{1}{x} )^n
$$
由于n是常数，因此当${x^\frac{1}{x}}$取得最大值时，乘积达到最大。所以问题就转化为求${y = x^\frac{1}{x}}$的极大值。求导：
$$
...
1. 循环求余法：
   ...
   $$
   利用此公司可以依次求得$x^1$,$x^2$,$x^3$,...,$x^a$对p的余数
